...
def check_1(lst_obj):
    ...
    Сложность: O(len(...)) сложность двух последовательных действий
    O(len(...)) + O(len(...)) = 2 * O(len(...)) = O(len(...))
    (в нашем случае min O(50), max O(10000))
    """
...
def check_2(lst_obj):
    ...
    Сложность: O(N^2). т.к. вложенные действия O(N) * O(N) = O(N^2).
    """
    for j in range(len(lst_obj)):          # O(N)
        if lst_obj[j] in lst_obj[j+1:]:    # O(N)
            return False                   # O(1) - только один вариант
    return True                            # O(1) - только один вариант
...
def check_3(lst_obj):
    ...
    Сложность: O(N log N) Cложность алгоритма оценивается наихудшим из
    слагаемых O(N) + O(N log N) + O(N) = O(N log N)
    """
