...
"""
Сначала я хотела сделать через именованные кортежи,
чтобы перебирать кортежи для дальнейших расчётов, придётся закинуть их, например, в список
но задача про расчёт среднего, а не про хранение, поэтому пришло в голову отправить всё в counter
Такое решение не даёт доступ к отдельным числам прибыли каждой компании,
но ТЗ этого и не подразумевает
"""
