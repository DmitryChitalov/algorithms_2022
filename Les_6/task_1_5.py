...
для оптимизации избавимся от создания списка в функции и используем f-строки для вывода
(вообще ещё можно заменить списки со словами на кортежи,
но в данном случае я этого не сделала, считаю, что тут должна сохраниться какая-то гибкость)
Кроме того, для красоты вынесла отдельно функцию генерирования шутки (при repeat=True),
чтобы не передавать в функцию именованные аргументы в kwargs, а только число и флаг

теперь по профилированию, посмотрим что там насоздавалось и какого оно размера
