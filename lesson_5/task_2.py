...
# 1  через collections defaultdict(list)
...
def sum_hexadecimal(op_1: str, op_2: str):
    ...
    print(f"Сумма чисел из примера: {list('%X' % sum_)},")
    mul_ = reduce(lambda x, y: x * y, [int(''.join(i), 16) for i in numbers.values()])
    print(f"произведение - ' {list('%X' % mul_)}.")
    return f'Сохранены как {list(numbers.values())}'
