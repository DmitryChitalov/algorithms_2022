...
# получение ошибки "KeyError: 0" (т.е. использование ключа, которого нет среди ключей данного отображения)
        ...
        total = end-start
        print(f'Среднее время {return_value}: {total/(args[0]):e} секунд.')
...
- список:  e-8 
- словарь: e-8
Сложность для списка и словаря совпадают, что подтверждается замером времени в большинстве случаев 
'''
@time_measure
...
- список:  e-8 
- словарь: e-8
Сложность для списка и словаря совпадают, однако эксперимент с замером времени этого не подтверждает 
'''
