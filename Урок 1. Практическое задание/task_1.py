...
def check_2(lst_obj):
    ...
    for j in range(len(lst_obj)):  # O(n) + O(1) + O(1) -> O(n)
        if lst_obj[j] in lst_obj[j + 1:]:  # O(n) + O(1) + O(1) + 1 -> O(n)
            return False  # O(1)
    return True  # O(1)
...
def check_3(lst_obj):
    ...
    Сложность: O(n) + O(n log n) + O(n) + O(n) + O(1) -> O(n)
    """
    lst_copy = list(lst_obj)  # O(n)
    lst_copy.sort()  # O(n log n)
    for i in range(len(lst_obj) - 1):  # O(n) + O(1) + O(1) - 1 -> O(n)
        if lst_copy[i] == lst_copy[i + 1]:  # O(n) + O(1) + O(1) + 1 -> O(n)
            return False  # O(1)
    return True  # O(1)
