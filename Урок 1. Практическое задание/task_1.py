...
2) определите сложность алгоритма в целом (Сложность: O(n**2)).
...
def check_3(lst_obj):
    ...
    Сложность: O(n) + O(n * log n) + O(n*2) = O(n * log n) - ИСПРАВЛЕНО
    """
    lst_copy = list(lst_obj)                 # O(n)
    lst_copy.sort()                          # O(n log n)
    for i in range(len(lst_obj) - 1):        # O(n) -
        if lst_copy[i] == lst_copy[i+1]:     # O(1) - сравнение элементов списков
            return False                     # O(1)
    return True                              # O(1)
#посмотрел таблицу сложностей и до сих пор не понимаю почему сложность не O(1), а O(n)
#Ведь получается O(len(..)) => O(1) 
