...
""" Возвращаем мы уже готовое множество поэтому О(1), сама по себе
функция set() имеет сложность O(len(...)) т.е зависит от входных
данных O(n)"""
...
"""if lst_copy[i] == lst_copy[i+1]: имеет сложность О(1)
после пересмотра занятия в этом уверен, т.к мы не сравниваем полностью 
2 списка, а имеем конкретные индексы елементов"""
