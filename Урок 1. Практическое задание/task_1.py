...
def check_2(lst_obj):
    ...
    for j in range(len(lst_obj)):          # O(n)
        if lst_obj[j] in lst_obj[j+1:]:    # O(1) + O(len(lst_obj) - j + 1) + O(n)
            return False                   # O(1)
    return True                            # O(1)
...
"""
    Узнал из дополнительных источников, 
    что общая оценка алгоритма 
    равна оценке самого сложного подалгоритма (если так можно сказать)
"""