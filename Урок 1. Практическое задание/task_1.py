...
def check_1(lst_obj):
    ...
    Сложность: #O(len(lst_obj)  (По самому ресурсному - линейное время)
    """
    lst_to_set = set(lst_obj)  # O(len(lst_obj)) - линейное время
...
def check_2(lst_obj):
    ...
    Сложность: #О(n2) T(n) = n + n + 1 + 1 = n + 2 = n2 - константная сложность
    """
    for j in range(len(lst_obj)):          # O(n) -линейное время
        if lst_obj[j] in lst_obj[j+1:]:    # O(n) -линейное время
