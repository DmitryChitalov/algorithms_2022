...
-- Сложность нужно указать только там, где есть 222
...
def check_2(lst_obj):
    ...
    Сложность: O(N) (O(N)+O(N)+O(1)+O(1)=O(N))
    """
    for j in range(len(lst_obj)):          # O(N) (O(N)+len(lst_obj)=O(N)+O(1)=O(N)
...
def check_3(lst_obj):
    ...
    Сложность: O(NlogN) (O(N)+O(NlogN)+O(N)+O(N)+O(1)+O(1)=O(N+NlogN+N+N+1+1)=O(NlogN))
    """
