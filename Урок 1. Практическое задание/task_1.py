...
def check_1(lst_obj):
    ...
    Сложность: T(N) = 2len(lst_obj)
    """
    lst_to_set = set(lst_obj)  # O(len(lst_obj))
    return lst_to_set  # O(len(lst_obj))
...
def check_2(lst_obj):
    ...
    Сложность: T(N) = 2N + 1 (либо False либо True)
    """
...
def check_3(lst_obj):
    ...
    Сложность: T(N) = 2 len(lst_obj) + N log N + N + 1
    """
    lst_copy = list(lst_obj)                 # O(len(lst_obj))
    lst_copy.sort()                          # O(N log N)
    for i in range(len(lst_obj) - 1):        # O(N)
        if lst_copy[i] == lst_copy[i+1]:     # O(len(lst_obj))
            return False                     # O(1)
    return True                              # O(1)
