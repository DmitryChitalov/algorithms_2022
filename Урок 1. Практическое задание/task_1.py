...
def check_2(lst_obj):
    ...
    for j in range(len(lst_obj)):          # O(n)
        if lst_obj[j] in lst_obj[j+1:]:    # O(len(lst_obj[j+1:])   # можно ли писать здесь O(n)?
            return False                   # O(1)
    return True                            # O(1)
...
def check_3(lst_obj):
    ...
    for i in range(len(lst_obj) - 1):        # O(len(lst_obj) - 1)  # можно ли писать здесь O(n)?
        if lst_copy[i] == lst_copy[i+1]:     # O(3)
            return False                     # O(1)
    return True                              # O(1)
