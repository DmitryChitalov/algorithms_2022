...
def check_1(lst_obj):
    ...
    Сложность: O(n),  линейная, т.к. мы не знаем размерности списка
    """
    lst_to_set = set(lst_obj)  # O(len(...)) = O(n)
    return lst_to_set  # O(1) - передается ссылка
...
def check_2(lst_obj):
    ...
    Сложность: O(n^2) <- O(n) * (O(n)+O(n)) + O(1) +  O(1) - квадратичная
    """
    for j in range(len(lst_obj)):          # O(len(...)) = O(n) - range
        if lst_obj[j] in lst_obj[j+1:]:    # O(n)+0(n) - срез + in
            return False                   # O(1)
    return True                            # O(1)
