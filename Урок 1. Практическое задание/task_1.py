...
def check_2(lst_obj):
    ...
    Сложность: O(n^2)  --> O(len(lst_obj)^2)
    """
    for j in range(len(lst_obj)):          # O(len(lst_obj))
        if lst_obj[j] in lst_obj[j+1:]:    # O(len(lst_obj[j+1:])
            return False                   # O(1)
    return True                            # 0(1)
...
def check_3(lst_obj):
    ...
    for i in range(len(lst_obj) - 1):        # O(n) --> O(len(lst_obj))
...
for j in (50, 500, 1000, 5000, 10000):          # O(5)
    ...
    # Всего 10 тыс. чисел
    lst = sample(range(-100000, 100000), j)    # O(j)
