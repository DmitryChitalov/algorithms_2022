...
def check_1(lst_obj):
    ...
    Сложность: O(n)"""
    lst_to_set = set(lst_obj)  # Создание множества. Сложность O(n).
    """Сложность зависит от длинны аргумента(итерируемой структуры) функции set.
    В рассматриваемом ниже len(lst) будеn равен 10000, как как обращение к функциям
    производится за пределами цикла, следовательно сложность равна O(10000)"""
    return lst_to_set  # Сложность O(1). Функция return имеет одинаковую сложность, вне зависимости от длины аргумента
...
def check_2(lst_obj):
    ...
    Сложность: O(N^2)
    """
    for j in range(len(lst_obj)):          # !!! Проход 0(n) после определения размера множества 0(1)
        if lst_obj[j] in lst_obj[j+1:]:    # !!! Проверка на входимость 0(n)
...
def check_3(lst_obj):
    ...
    Сложность: !!! O(2N)
    """
    lst_copy = list(lst_obj)                 # !!! Создание списка 0(N)
    lst_copy.sort()                          # !!! Выполнение сортировки 0(N logn N)
    for i in range(len(lst_obj) - 1):        # !!! Проход 0(N)
        if lst_copy[i] == lst_copy[i+1]:     # !!! Сравнение элемента 0(1)
            return False                     # !!! У функции return константная сложность O(1) в худшем случае не выполнится
    return True                              # !!! У функции return константная сложность O(1)
