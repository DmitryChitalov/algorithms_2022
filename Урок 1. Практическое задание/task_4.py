...
def view_users():
#O(N)
    print(f'введите id пользователя для прооверки')
    ...
    if user:
        if not user[2]:
            print(f'{user[0]} - пройдите активацию')
            question = input(f'Активируем аккаунт {user[0]}? да/нет\n')
            if question.lower() in ['да', 'yes']:  # O(N)
        ...
        print(f'Пользователя под таким id - {id_user} не существует')
