...
# Вариант решения №2 дает сложность #O(n(2n)) в отличии от первого решения и
# является гораздо лучшим вариантов таким образом
...
def log_in(login, password, log_pass_status):
    for i in list(user_info.keys()):  # O(n)
        if 'login' in user_info[i]:  # O(1)
            if user_info[i].get('login') == login and user_info[i].get('password') == password:  # O(2n)
                ...
                if user_info[i].get('status') == 'Activated':  # O(1)
                    log_pass_status[1] = 1  # O(1)
                    print("Доступ разрешен")  # O(1)
                    break  # O(1)
                else:
                    print("Требуется активация аккаунта")  # O(1)
                    break  # O(1)
    if log_pass_status == [0, 0]:  # O(n)
        print("Неправильная пара логин/пароль")  # O(1)


log_in(input("Введите логин: "), input("Введите пароль: "), [0, 0])  # O(len(3))
