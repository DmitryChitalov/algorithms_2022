...
class TaskBoard:
    ...
    def resolve_task(self):
        ...
        print('Закрываем текущую задачу и добавляем в лог')

    def to_revision_task(self):
        task = self.cur_queue.from_queue()
        self.revision_queue.to_queue(task)
        print('Отправляем текущую задачу на доработку')

    def to_current_queue(self, item):
        self.cur_queue.to_queue(item)
        print('Добавляем задачу в текущие')

    def from_revision(self):
        task = self.revision_queue.from_queue()
        self.cur_queue.to_queue(task)
        print('Возвращаем задачу из доработки в текущую очередь')

    def current_task(self):
        return self.cur_queue.elems[len(self.cur_queue.elems) - 1], print('Текущая задача')

    def current_revision(self):
        return self.revision_queue.elems[len(self.revision_queue.elems) - 1], print('Задача в доработке')
