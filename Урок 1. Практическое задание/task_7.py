...
# есть два варианта
# первый, просто не переносить пробелы
def pal_checker_v1(string: str):
    dc_obj = DequeClass()
    for el in string:
        if el!=' ':
...
# Если извлекли пробел, то извлекаем дальше пока не будет получен не пробел (или строка не кончится)
def pal_checker_v2(string: str):
    dc_obj = DequeClass()
    for el in string:
        dc_obj.add_to_rear(el)
    ...
    while dc_obj.size() > 1 and still_equal:
        first = dc_obj.remove_from_front()
        while dc_obj.size() > 1 and first == ' ':
            first = dc_obj.remove_from_front()
        last = dc_obj.remove_from_rear()
        while dc_obj.size() > 1 and last == ' ':
            last = dc_obj.remove_from_rear()
        if first != last:
            still_equal = False
