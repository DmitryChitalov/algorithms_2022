...
def db_password_checker(password):
    """
    Проверяет, есть ли в таблице users соответствующий хеш
    """
...
def db_password_add(password_hash):
    """
    Добавляет в таблицу users запись с хешем.
    """
    db_connection = sqlite3.connect('db_password.sqlite')
    cursor = db_connection.cursor()
    sql_password_add_request = f"INSERT INTO users (hash)VALUES ('{password_hash}');"
    cursor.execute(sql_password_add_request)
...
def authorization():
    """
    Запрашивает пароль, заносит пароль в бд, повторно запрашивает пароль, проверяет его наличие в бд.
    """
    ...
    if db_password_checker(second_password):
        db.close()
