...
class Profit:
    ...
    def company_profit(self):
        ...
        Company = namedtuple('Company', 'qrtr quarter_1 quarter_2 quarter_3 quarter_4')
    ...
    def profit_of_company(self, qrtr):
        self.comp_profit = qrtr.quarter_1 + qrtr.quarter_2 + qrtr.quarter_3 + qrtr.quarter_4
    ...
    def compare_profits(self):
        profit_max = 'Предприятия, с прибылью выше среднего значения:'
        profit_min = 'Предприятия, с прибылью ниже среднего значения:'
        for i in self.companies:
            if self.profit_of_company(i) > self.sum / self.count:
                profit_max += f' {i.qrtr},'
            if self.profit_of_company(i) < self.sum / self.count:
                profit_min += f' {i.qrtr},'
        profit_max = profit_max[:-1]
        profit_min = profit_min[:-1]
        print(profit_max)
        print(profit_min)
