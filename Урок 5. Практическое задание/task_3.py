...
# В результате 4х проходов, операция list append выполняется незначительно медленнее чем deque append
...
# В результате 4х проходов, операция list pop выполняется незначительно медленнее чем deque pop
...
# За четыре прохода, операция list extend выполняется в среднем в два раза медленне чем deque extend.
...
# За 4 прохода операция list insert выполняется гораздо медленнее, чем deque appendleft
...
# За четыре прохода операция list popleft выполняется гораздо медленнее,  чем deque popleft
...
# За четыре прохода операция  list extendleft выполняется гораздо медленнее, чем deque extendleft
...
# 3) в сравнении операции получения элемента списка и дека по индексу - время выполнения для списка и дека примерно
# одинаковое .

# Таким образом, если необходимо использовать операции appendleft, popleft, extendleft - то оптимально будет
# пользоваься деком .
